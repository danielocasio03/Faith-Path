...
class HomeVC: UIViewController {
	...
	func themeOTDHandler() {
		...
		themeOTDManager.fetchAllThemeContent(theme: "\(DesignManager.shared.currentTheme)")
			.sink { completion in
				switch completion {
	...
	func updateThemeOTD(itemData: [ThemeOTDModel]) {
		...
		//Takes the theme name/category and sets it to the section label
		homeView.themeOTDView.themeSectionLabel.text = "Today's Theme - \(itemData[0].verse_category)"
	...
	func viewSetup() {
		...
		homeView.faithPathView.pathwayView.bibleButton.addTarget(self, action: #selector(octaBibleTapped), for: .touchUpInside)
